...
namespace StockAnalyzer
{
    public static class StockUtils
    {
        ...
        public static void DisplayStockInfo(Tuple<string, StockData[]> stockHistories, long elapsedTime)
        {
            var timeElapsed = $"Time elapsed {elapsedTime} ms";

            var legendText = stockHistories.Item1;

            var highest = stockHistories.Item2.OrderByDescending(f => f.High).First();
            var lowest = stockHistories.Item2.OrderBy(f => f.Low).First();
